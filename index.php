...
<div class="d-flex vh-100" style="background-color: rgb(170, 159, 91)">
    ...
    <div class="d-flex col-4">
        ...
        ");
        } else if (isset($_GET['subject_search']) && isset($_GET['session-filter']) && isset($_COOKIE['email']) && isset($_GET['bill_number']) && !isset($_GET['user_question'])) {
            ...
            echo "<input type='hidden' id='session-filter' name='session-filter' value='$session_filter'>";
            ...
            <div class='rounded-5 h-75 text-dark text-wrap m-2 p-3' style='background-color: lightgrey'>
            <br>
                <p text-align: center>Hello! I am the AI Lawyer. I can help you with any legal questions you may have.<p>
                <br>
                <h4 style='text-align:center'>Please select a bill to ask a question.</h4>
             </div>
        </div>
            ";
        }
    ...
    <div class="d-inline-flex col-8 p-4">
        <div class="w-100 bg-white rounded-5 p-2">
            ...
            <div name="search_results" class="bg-secondary h-75 overflow-auto">
                ...
                if (isset($_GET['subject_search']) && isset($_GET['session-filter']) && !isset($_COOKIE['email']) && !isset($_GET['bill_number'])) {
                ...
                } else if (isset($_GET['subject_search']) && isset($_GET['session-filter']) && isset($_COOKIE['email']) && !isset($_GET['bill_number'])) {
                    $subject_search = $_GET['subject_search'];
                    $session_filter = $_GET['session-filter'];
                    $subject_search = str_replace('_', ' ', $subject_search);
                    $query = "SELECT * FROM law_table2 WHERE bill_summary is not NULL AND session='$session_filter' AND subject='$subject_search' LIMIT 100";
                    ...
                    if ($result) {
                        if (pg_num_rows($result) > 0) {
                            ...
                            while ($row = pg_fetch_assoc($result)) {
                                ...
                                display: inline-block;' href='index.php?session-filter=$session_filter&subject_search=$subject_search&bill_number=$bill_number'>Ask a question about this bill.</a></h4></center>";
                                echo '</div>';
                                echo '</div>';
                                echo '</div>';
                                $i++;
                    ...
                    $bill_number2 = $_GET['bill_number'];
                    $subject_search = str_replace('_', ' ', $subject_search);
                    $query = "SELECT * FROM law_table2 WHERE bill_summary is not NULL AND session='$session_filter' AND subject='$subject_search' AND bill_number='$bill_number2' LIMIT 100";
                    $result = pg_query($conn, $query);
                    if ($result) {
                        if (pg_num_rows($result) > 0) {
                            ...
                            while ($row = pg_fetch_assoc($result)) {
                                ...
                                text-align: center;
                                text-decoration: none;
                                display: inline-block;' href='index.php?session-filter=$session_filter&subject_search=$subject_search'>Return to list of search results.</a></h4></center>";
                                $i++;
                            }
                            echo ("</div>");
                        } else {
